...
async function uploadImage(localUri: string, folder: string): Promise<UploadResponse> {
    // 1. Validar que tenemos la URI local
    if (!localUri) {
        console.error("Cloudinary Service: Se llamó a uploadImage sin una URI local.");
    ...
    try {
        ...
        console.error('🔴 [Cloudinary] ERROR AL OBTENER LA FIRMA:', JSON.stringify(error, null, 2));
        Alert.alert('Error de Conexión', 'No se pudo comunicar con el servidor para autorizar la subida de la imagen.');
    ...
    try {
        console.log('[Cloudinary] Paso 3: Subiendo imagen directamente a Cloudinary...');
        ...
        Alert.alert('Error de Subida', 'No se pudo subir la imagen a la nube. Revisa la consola para más detalles.');
